...
module.exports = function (SerialPort) {
    ...
    modem.port = SerialPort(device, options, (error) => {
      if (error) {
        ...
        let result = {
          ...
          data: { modem: modem.port.path, status: 'Online' },
        };
        modem.device = device;
  ...
  modem.readSMSById = function (id) {
    ...
    item.logic = (newpart) => {
      ...
      if (regx.test(newpart)) {
        ...
        if (modem.enableConcatenation && message.udh) {
          if (modem.tempConcatenatedMessages[message.udh.referenceNumber]) {
            modem.tempConcatenatedMessages[message.udh.referenceNumber].push(
              message
            );
          } else {
            modem.tempConcatenatedMessages[message.udh.referenceNumber] = [
              message,
            ];
          }
          let tempMessage = self.arrangeMessages(
            modem.tempConcatenatedMessages[message.udh.referenceNumber]
          )[0];
          //check if complete
      ...
      if ((newpart === '>' || newpart === 'OK') && resultData) {
        ...
        newpart.includes('ERROR') ||
        ((newpart === '>' || newpart === 'OK') && !resultData)
      ) {
        return {
          resultData: {
            status: 'ERROR',
      ...
      (resultCheck, error) => {
        if (!error) {
          modem.enableEcho(
            (result, error) => {
              if (!error) {
                modem.checkPINRequired(
                  (result, error) => {
                    if (
                      !error &&
                      result.data &&
                      result.data.pinNeeded &&
                      modem.pin.length
                    ) {
                      modem.providePIN(
                        modem.pin,
  ...
  modem.resetModem = function (callback, priority, timeout) {
    ...
    const item = modem.executeCommand(
      ...
      undefined,
      true
    ); // auto activate it if no Echo is active
    item.logic = (newpart) => {
  ...
  modem.sendSMS = function (number, message, alert = false, callback) {
    ...
    try {
      if (number && message) {
        ...
        // const parts = pdu.generateSubmit(number, message);
        //use node-pdu
        ...
        for (let i = 0; i < parts.length; i++) {
          modem.executeCommand(
            `AT+CMGS=${parts[i].toString().length / 2 - 1}`,
            (data) => {},
            false,
            100
          );
          modem.executeCommand(
            `${parts[i].toString()}` + '\x1a',
            function (data, error) {
              // modem.executeCommand(`AT+CMGS=${parts[i].length}`, (data) => { }, false, 100)
        ...
        return {
          resultData: {
            ...
            data: {
              ...
              signalStrength:
                signal[0] !== 99 ? 113 - signal[0] * 2 : undefined,
            },
          },
          returnResult: true,
        };
  ...
  modem.dataReceived = (buffer) => {
    ...
    parts.forEach((part) => {
      ...
      newparts.forEach((newpart) => {
        ...
        if (modem.incomingSMSIndication && newpart.startsWith('+CMTI:')) {
          // New Message Indicator with SIM Card ID, After Recieving Read The Message From the SIM Card
          const splitted_newpart = newpart.split(',');
          modem.readSMSById(splitted_newpart[1]);
        } else if (
          modem.incomingCallIndication &&
          newpart.startsWith('+CLIP')
        ) {
          // New Incomming call
          const splitted_newpart = newpart.split(',');
        ...
        if (modem.queue.length && modem.queue[0]) {
          if (modem.queue[0].command.trim() === newpart.trim()) {
            // Echo of command received, only process if active
            modem.queue[0].inProgress = true;
            modem.logger.debug('Activate Message Processing for: ' + newpart);
          }
          if (modem.queue[0].status === 'sendSMS') {
            // If SMS is currently Sending Emit currently sending SMS
          ...
          if (
            modem.queue[0].command.startsWith('AT+CMGS') ||
            pduTest.test(modem.queue[0].command)
          ) {
            // Sending of Message if with response ok.. Then Message was sent successfully.. If Error then Message Sending Failed
            resultData = {
              status: 'success',
              request: 'SendSMS',
              data: {
            ...
            // let's check if it has a logic function
          ...
          let callback;
          if (returnResult && modem.queue[0].inProgress) {
            // Expected Result was ok or with error call back function that asked for the data or emit to listener, Execute next Command if any or Execute Next Command if TIME Out and modem did not respond
            returnResult = false;
            if (modem.queue[0] && modem.queue[0].callback) {
              callback = modem.queue[0].callback;
              modem.logger.debug(
                'Call callback for: ' + modem.queue[0].command
              );
            } else {
              callback = null;
  ...
  modem.getSimInbox = (callback, timeout = 15000) => {
    ...
    const item = modem.executeCommand(
      modem.modemMode === 0 ? `AT+CMGL=4` : `AT+CMGL="ALL"`,
      (result, error) => {
        callback(result, error);
      },
      false,
  ...
  self.processMessage = (message, messageIndex) => {
    let newMessage = {
      ...
      index: messageIndex,
      dateTimeSent: message.getScts()
        ? new Date(message.getScts().getIsoString())
        : null || null,
      header: {
  ...
  self.processConcatenatedMessage = (newMessageResult, message) => {
    let target = newMessageResult.find(
      (a) => a.udh && a.udh.referenceNumber === message.udh.referenceNumber
    );
    if (target) {
      target.udhs.push({
        index: message.index,
