...
function Modem(port, options) {
  if (!(this instanceof Modem)) return new Modem(port, options);
  var defaults = {
    retry: 0,
    timeout: 5000,
    dataBits: 8,
...
Modem.prototype.sms_send_pdu = function (pdu) {
  // temporary disable retry.
  return this.set("CMGS", pdu.length, { retry: 0 }).then((x) => {
    return this.send(pdu.hex + "\u001a");
  });
};
